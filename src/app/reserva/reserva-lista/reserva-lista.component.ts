...
export class ReservaListaComponent implements OnInit {
  ...
  obtenerReservasEstados(estado: string): void {
     this.reservas = [];
     if(estado != 'todas') {
        this.reservaService.obtenerReservasPorEstado(this.idPropiedad, estado)
          .subscribe((reservasEstado) => {
                        this.reservas = reservasEstado;
                      },
                      (error) => {
                        console.log(error);
